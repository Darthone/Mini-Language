...
def p_list_stuff_expr_list ( p ) :
    'list_stuff : LBRACKET expr_list RBRACKET'
    p[0] = ListStuff(p[2]);

def p_list_stuff_brackets ( p ) :
    'list_stuff : LBRACKET RBRACKET'
    p[0] = EmptyList()

def p_list_stuff_car ( p ) :
    'list_stuff : CAR expr'
    p[0] = Car(p[2])

def p_list_stuff_cdr ( p ) :
    'list_stuff : CDR expr'
    p[0] = Cdr(p[2])

def p_list_stuff_cons ( p ) :
    'list_stuff : CONS expr COMMA expr'
    p[0] = Cons(p[2], p[4])

def p_list_stuff_null ( p ) :
    'list_stuff : NULL expr'
    p[0] = Null(p[2])
